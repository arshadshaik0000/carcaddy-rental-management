...
public class ClientController {
	...
	public String getRent(Model model) {
		...
		try {
			ResponseEntity<List<Car>> response = restTemplate.exchange(BASE_URL + "/viewAllCars",
					HttpMethod.GET, null,
	...
	public String bookCar(@PathVariable int carId,
			@ModelAttribute("booking") Rent_Booking booking, BindingResult result,
			Model model) {
		RestTemplate restTemplate = new RestTemplate();
		try {
			if (booking != null) {
				...
				Employee emp = restTemplate.getForObject(BASE_URL + "/cars/" + carId + "/employee",
						Employee.class);
				...
				ResponseEntity<Rent_Booking> response = restTemplate.postForEntity(
						BASE_URL + "/bookCar",
						book,
						Rent_Booking.class);
	...
	public String viewBookingById(@RequestParam("bookingId") String bookingId, Model model) {
		RestTemplate restTemplate = new RestTemplate();
		try {
			ResponseEntity<Rent_Booking> response = restTemplate.getForEntity(
	...
	public String showHistory(Model model) {
		...
		try {
			ResponseEntity<List<Rent_Booking>> response = restTemplate.exchange(BASE_URL + "/viewAllBookings",
					HttpMethod.GET, null,
					new ParameterizedTypeReference<List<Rent_Booking>>() {
					});
	...
	public String handleUpdateBooking(
			@PathVariable int bookingId,
			@ModelAttribute("booking") Rent_Booking booking,
			BindingResult result,
			Model model) {

		RestTemplate restTemplate = new RestTemplate();
		try {
			ResponseEntity<Rent_Booking> response1 = restTemplate.getForEntity(
					BASE_URL + "/viewBookingById/" + bookingId,
					Rent_Booking.class);
			...
			ResponseEntity<String> response = restTemplate.exchange(
					BASE_URL + "/updateBooking/" + bookingId,
					HttpMethod.PUT,
					requestEntity,
					String.class);
